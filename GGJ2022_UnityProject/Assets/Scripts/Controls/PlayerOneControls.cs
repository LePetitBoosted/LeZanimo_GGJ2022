...
public class PlayerOneControls : MonoBehaviour
{
    ...
    private void Update()
    {
        if (hasInput == true)
        {
            horizontalMove = Input.GetAxis("Horizontal P1");
            ...
            if (Input.GetButtonDown("Jump P1") && isGrounded) 
            {
                Jump();
            }

            if (Input.GetButtonDown("Dash P1") && dashAvailable)
            {
                GetInputRaw();

                Vector2 dashDir;
                if (Mathf.Abs(Input.GetAxis("Horizontal P1")) > 0 || Mathf.Abs(Input.GetAxis("Vertical P1")) > 0)
                {
                    if (Mathf.Abs(Input.GetAxis("Horizontal P1")) > Mathf.Abs(Input.GetAxis("Vertical P1")))
                    {
