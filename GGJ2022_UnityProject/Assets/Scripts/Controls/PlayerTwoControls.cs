...
public class PlayerTwoControls : MonoBehaviour
{
    ...
    private void Update()
    {
        if (hasInput == true)
        {
            horizontalMove = Input.GetAxis("Horizontal P2");
            ...
            if (Input.GetButtonDown("Dash P2") && dashAvailable)
            {
                GetInputRaw();

                Vector2 dashDir;
                if (Mathf.Abs(Input.GetAxis("Horizontal P2")) > 0 || Mathf.Abs(Input.GetAxis("Vertical P2")) > 0)
                {
                    if (Mathf.Abs(Input.GetAxis("Horizontal P2")) > Mathf.Abs(Input.GetAxis("Vertical P2")))
                    {
